...
- (NSString*) descriptionForObjCCall
{
    ...
    if(![selectorParam.type isEqualToString:@Raygun_KSCrashMemType_String])
    {
        return nil;
    }
    ...
    for(int paramNum = 0; paramNum < paramCount; paramNum++)
    {
        ...
        if(paramNum < 2)
        {
            ...
            if(param.value != nil)
            {
                if([param.type isEqualToString:@Raygun_KSCrashMemType_String])
                {
...
@implementation Raygun_KSCrashDoctor
...
{
    return [report objectForKey:@Raygun_KSCrashField_RecrashReport];
}

- (NSDictionary*) systemReport:(NSDictionary*) report
{
    return [report objectForKey:@Raygun_KSCrashField_System];
}

- (NSDictionary*) crashReport:(NSDictionary*) report
{
    return [report objectForKey:@Raygun_KSCrashField_Crash];
}

- (NSDictionary*) infoReport:(NSDictionary*) report
{
    return [report objectForKey:@Raygun_KSCrashField_Report];
}

- (NSDictionary*) errorReport:(NSDictionary*) report
{
    return [[self crashReport:report] objectForKey:@Raygun_KSCrashField_Error];
}

- (CPUFamily) cpuFamily:(NSDictionary*) report
{
    NSDictionary* system = [self systemReport:report];
    NSString* cpuArch = [system objectForKey:@Raygun_KSCrashField_CPUArch];
...
- (NSString*) mainExecutableNameForReport:(NSDictionary*) report
{
    ...
    return [info objectForKey:@Raygun_KSCrashField_ProcessName];
}

- (NSDictionary*) crashedThreadReport:(NSDictionary*) report
{
    NSDictionary* crashReport = [self crashReport:report];
    NSDictionary* crashedThread = [crashReport objectForKey:@Raygun_KSCrashField_CrashedThread];
    if(crashedThread != nil)
    {
        return crashedThread;
    }

    for(NSDictionary* thread in [crashReport objectForKey:@Raygun_KSCrashField_Threads])
    {
        if([[thread objectForKey:@Raygun_KSCrashField_Crashed] boolValue])
        {
            return thread;
        }
    }
...
{
    NSDictionary* backtrace = [threadReport objectForKey:@Raygun_KSCrashField_Backtrace];
    return [backtrace objectForKey:@Raygun_KSCrashField_Contents];
}

- (NSDictionary*) basicRegistersFromThreadReport:(NSDictionary*) threadReport
{
    NSDictionary* registers = [threadReport objectForKey:@Raygun_KSCrashField_Registers];
    NSDictionary* basic = [registers objectForKey:@Raygun_KSCrashField_Basic];
    return basic;
}
...
- (NSDictionary*) lastInAppStackEntry:(NSDictionary*) report
{
    ...
    {
        NSString* objectName = [entry objectForKey:@Raygun_KSCrashField_ObjectName];
...
{
    NSDictionary* machError = [errorReport objectForKey:@Raygun_KSCrashField_Mach];
    if(machError != nil)
    {
        NSString* exceptionName = [machError objectForKey:@Raygun_KSCrashField_ExceptionName];
        return [exceptionName isEqualToString:@"EXC_BAD_ACCESS"];
    }
    NSDictionary* signal = [errorReport objectForKey:@Raygun_KSCrashField_Signal];
    NSString* sigName = [signal objectForKey:@Raygun_KSCrashField_Name];
...
{
    NSDictionary* crashedThread = [self crashedThreadReport:report];
    NSArray* notableAddresses = [crashedThread objectForKey:@Raygun_KSCrashField_NotableAddresses];
    for(NSDictionary* address in [notableAddresses objectEnumerator])
    {
        NSString* type = [address objectForKey:@Raygun_KSCrashField_Type];
        if([type isEqualToString:@"string"])
        {
            NSString* value = [address objectForKey:@Raygun_KSCrashField_Value];
    ...
    {
        NSString* objectName = [entry objectForKey:@Raygun_KSCrashField_ObjectName];
        NSString* symbolName = [entry objectForKey:@Raygun_KSCrashField_SymbolName];
...
- (KSCrashDoctorFunctionCall*) lastFunctionCall:(NSDictionary*) report
{
    ...
    function.name = [lastStackEntry objectForKey:@Raygun_KSCrashField_SymbolName];

    NSDictionary* crashedThread = [self crashedThreadReport:report];
    NSDictionary* notableAddresses = [crashedThread objectForKey:@Raygun_KSCrashField_NotableAddresses];
    ...
    for(NSString* regName in regNames)
    {
        ...
        {
            param.type = [notableAddress objectForKey:@Raygun_KSCrashField_Type];
            NSString* className = [notableAddress objectForKey:@Raygun_KSCrashField_Class];
            NSString* previousClass = [notableAddress objectForKey:@Raygun_KSCrashField_LastDeallocObject];
            NSString* value = [notableAddress objectForKey:@Raygun_KSCrashField_Value];

            if([param.type isEqualToString:@Raygun_KSCrashMemType_String])
            {
                param.value = value;
            }
            else if([param.type isEqualToString:@Raygun_KSCrashMemType_Object])
            {
                param.className = className;
                param.isInstance = YES;
            }
            else if([param.type isEqualToString:@Raygun_KSCrashMemType_Class])
            {
                param.className = className;
                param.isInstance = NO;
            }
...
{
    NSDictionary* stack = [crashedThreadReport objectForKey:@Raygun_KSCrashField_Stack];
    return [[stack objectForKey:@Raygun_KSCrashField_Overflow] boolValue];
}

- (BOOL) isDeadlock:(NSDictionary*) report
{
    NSDictionary* errorReport = [self errorReport:report];
    NSString* crashType = [errorReport objectForKey:@Raygun_KSCrashField_Type];
    return [@Raygun_KSCrashExcType_Deadlock isEqualToString:crashType];
}
...
- (NSString*) diagnoseCrash:(NSDictionary*) report
{
    @try
    {
        NSString* lastFunctionName = [[self lastInAppStackEntry:report] objectForKey:@Raygun_KSCrashField_SymbolName];
        ...
        NSString* crashType = [errorReport objectForKey:@Raygun_KSCrashField_Type];
        if([crashType isEqualToString:@Raygun_KSCrashExcType_NSException])
        {
            NSDictionary* exception = [errorReport objectForKey:@Raygun_KSCrashField_NSException];
            NSString* name = [exception objectForKey:@Raygun_KSCrashField_Name];
            NSString* reason = [exception objectForKey:@Raygun_KSCrashField_Reason]? [exception objectForKey:@Raygun_KSCrashField_Reason]:[errorReport objectForKey:@Raygun_KSCrashField_Reason];
        ...
        {
            uintptr_t address = (uintptr_t)[[errorReport objectForKey:@Raygun_KSCrashField_Address] unsignedLongLongValue];
            if(address == 0)
            {
